...
## Module 4 Checkpoint 02: deterministic button behavior

The Module 4 Flask factory supports injected scraper and loader test doubles.
When those test doubles are supplied, `POST /pull-data` passes the rows returned
by the fake scraper directly to the fake loader and returns JSON, so tests never
need live internet access. Both `/pull-data` and `/update-analysis` return HTTP
409 with `{"busy": true}` while Pull Data is active. The update endpoint performs
no analysis refresh while busy. Production Pull Data still uses the inherited
...
## Module 4 Checkpoint 03: analysis formatting and PostgreSQL tests

Checkpoint 03 adds deterministic formatting tests and real PostgreSQL write tests. Database tests create a temporary schema for each test, exercise the Flask `POST /pull-data` endpoint with a fake scraper and a real PostgreSQL loader, verify the required Module 3 fields, and drop the temporary schema after the test. The production `public.applicants` table is not modified by these tests.

The database layer accepts either `DATABASE_URL` (preferred for CI) or the standard PostgreSQL `PGHOST`, `PGPORT`, `PGDATABASE`, `PGUSER`, and `PGPASSWORD` environment variables. No credentials are committed to source control.
